...
//			allowBlank : true,
		...
		propItems.push({
			...
			checked: false,
			hideLabel: true,
			name: "useFixedAddressBook",
			boxLabel: t("Use fixed address book"),
			listeners: {
				check: function(field, checked) {
					this.selectAddressbook.setDisabled(!checked);
					if(!checked) {
						this.selectAddressbook.setValue(null);
					}
				},
				scope: this
			}
		...
		propItems.push(this.selectAddressbook = new go.modules.community.addressbook.AddresBookCombo({
...
//		this.costCodesGrid = new GO.billing.CostCodesGrid();
		...
		this.formPanel.form.on("load", function(form) {
			form.findField('useFixedAddressBook').setValue(!!this.selectAddressbook.getValue());
		}, this);
