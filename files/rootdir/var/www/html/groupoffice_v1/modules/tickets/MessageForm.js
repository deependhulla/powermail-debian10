...
//	config.modal=true;
	...
	if(GO.tickets.status_per_ticket_type) {
		this.selectType.on('select', function(combo, record) {
			this.selectStatus.store.baseParams.type_id = record.data.id;
			this.selectStatus.getStore().load();
		}, this);
	}
			...
			if(!this.selectType.store.loaded){
				this.selectType.store.load({
					callback:function(){
						this.selectType.setValue(this.ticketData.type_id);
						if(GO.tickets.status_per_ticket_type) {
							this.selectStatus.store.baseParams.type_id = this.ticketData.type_id;
							this.selectStatus.getStore().load({
								callback:function(){
